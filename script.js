...
let allTitles = ["Good luck beating this.", "This is where shit gets real.", "I dare you to beat it.", "Have you found the secret code yet?"]

function initialize() {
    title.title = allTitles[Math.floor(Math.random() * allTitles.length)]
}

function LetAIGoFirst() {
    let initialBoard = document.getElementsByClassName('cell')
    //let initialMove = findBestMove(initialBoard)
    let initialMove = Math.floor(Math.random() * 9);
    initialBoard[initialMove].innerHTML = 'O' // Apparently all moves are equally good
    movesLeft--
}
...
function retry() {
    ...
    if (checkbox.checked == true) { LetAIGoFirst(); }
}
